# AICluster v2.0.0 â€” Troubleshooting Guide
...
**Verification:** Run `python --version` from a new terminal â€” it should return `Python 3.12.x`.
...
**Cause:** The Visual C++ 2015â€“2022 Redistributable (x64) is not installed.
...
**Verification:** Run the AICluster installer again â€” the VC++ check passes.

---

### 1.4 Windows SmartScreen Blocks Installation

**Symptoms:** A blue dialog appears: "Windows protected your PC" â€” SmartScreen prevented an unrecognized app from starting.
...
**Verification:** Re-run the installer â€” no files are quarantined.
...
1. Use a download manager that supports resume (e.g., `curl -C -`):
   ```
   curl -C - -O https://releases.aicluster.io/v2.0.0/AICluster-1.3.1-setup.exe
   ```
...
### 1.10 Corrupted Installer â€” "Setup file is damaged"
...
1. Verify the config file exists:
   ...
   ```
2. Check which process is using port 8000 (default):
   ```
...
**Verification:** `netstat -ano | findstr :8000` shows only `aicluster.exe` listening.
...
**Verification:** The master starts without JWT errors. Existing tokens are invalidated â€” all workers must re-authenticate.
...
2. If the CLI tool itself is unavailable, truncate the `users` table in the database:
   ...
   ```
   Then restart the master â€” a new admin account is created with defaults (admin / admin).

**Verification:** Log into the web UI at `https://localhost:8000` with the new credentials.
...
**Verification:** Reload the UI â€” all API requests succeed without CORS errors.
...
3. Check the firewall is not blocking port 8000.
...
4. Apply the hotfix patch from the AICluster v2.0.0 patch-1 release.
...
1. Verify the master is running:
   ```
   curl -k https://<master-ip>:8000/health
   ```
2. Check the worker config:
   ```
   type "C:\ProgramData\AICluster\worker.yaml"
   ```
   Ensure `master_url` is correct (e.g., `https://192.168.1.100:8000`).
3. Test connectivity from the worker to the master:
   ```
   Test-NetConnection <master-ip> -Port 8000
   ```
...
**Verification:** The worker authenticates successfully â€” log shows `authenticated as worker-01`.
...
**Cause:** The worker is using an API path that does not exist â€” version mismatch between worker and master.
...
2. Check the worker version:
   ...
   ```
3. Upgrade both to v2.0.0 so API paths align.

**Verification:** Registration succeeds â€” log shows `registered successfully`.
...
1. Verify the worker process is running:
   ...
   ```
2. Check the heartbeat interval â€” ensure it matches the master's expected interval.
...
**Verification:** Re-submit the job â€” it completes with exit code 0.
...
**Cause:** Windows Firewall or a third-party firewall is blocking port 8000 (or the configured port).
...
2. Add an inbound rule for the AICluster port:
   ```
   netsh advfirewall firewall add rule name="AICluster Master" dir=in action=allow protocol=TCP localport=8000
   ```
3. Add an outbound rule on worker machines:
   ```
   netsh advfirewall firewall add rule name="AICluster Worker Outbound" dir=out action=allow protocol=TCP remoteport=8000
   ```

**Verification:** `Test-NetConnection <master-ip> -Port 8000` succeeds from the worker.
...
2. Update `worker.yaml`:
   ```yaml
   master_url: "https://192.168.1.100:8000"
   ```
...
**Symptoms:** `context length exceeded` â€” the input text exceeds the model's maximum token limit.
...
**Verification:** The master starts successfully â€” log shows `migrations completed`.
...
| `netstat -ano \| findstr :8000` | Check which process owns port 8000 |
| `certutil -hashfile <file> SHA256` | Verify file integrity |

---

*Document version 1.0 â€” AICluster v2.0.0*
