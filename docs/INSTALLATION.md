# AICluster v1.3.1 Installation Guide

## Introduction

### What is AICluster?

AICluster transforms a local area network (LAN) of Windows computers into a unified AI compute cluster. It runs entirely offline — no internet connection is required after initial setup.
...
| **CLI** | Command-line tools for scripting and automation. | — |
| **Installer** | Single-file setup wizard (Inno Setup). | — |
...
Download `AIClusterSetup-1.3.1.exe` from the [GitHub Releases page](https://github.com/manojkumar12-maker/AICluster-Studio/releases).
...
Download `AICluster-1.3.1-portable.zip` from the releases page.
...
### Method 1: Installer (Easiest — Recommended for Most Users)
...
Double-click `AIClusterSetup-1.3.1.exe`.
...
- **Port 8000** (Master Server) — Inbound rule
- **Port 3000** (Web Dashboard) — Optional inbound rule
...
├── master\
│   └── AIClusterMaster.exe          # Master server executable
├── worker\
│   └── AIClusterWorker.exe          # Worker executable
├── studio\
│   └── AIClusterStudio.exe          # Desktop IDE
├── master-control\
│   └── MasterControlCenter.exe      # Cluster management desktop app
├── worker-control\
│   └── WorkerControlCenter.exe      # Worker management desktop app
├── cli\
│   └── aicluster.exe                # Command-line tools
├── plugins\                         # User-installed plugins (empty initially)
├── models\                          # Local LLM models (empty initially)
├── logs\                            # Runtime logs (created on first run)
├── config\
│   ├── default.yaml                 # Default configuration
│   ├── development.yaml             # Development overrides
│   └── production.yaml              # Production overrides
├── data\                            # Runtime data (SQLite DB, secrets)
├── assets\
│   ├── manifest.json                # App manifest
│   └── icons\                       # Application icons
├── AIClusterMaster.exe              # Convenience shortcut
├── AIClusterWorker.exe              # Convenience shortcut
...
> **⚠️ CRITICAL**: Save this password. You will need it to log in. The password is shown only once on the first startup. If you lose it, delete the database and restart to generate a new password.
...
| `AICLUSTER_MASTER_SECRET` | — | Shared secret for worker authentication |
...
├── aicluster.log          # Main log file
├── aicluster.log.1        # Rotated log
├── aicluster.log.2        # Rotated log
├── aicluster.log.3        # Rotated log
├── aicluster.log.4        # Rotated log
└── aicluster.log.5        # Rotated log
```
...
> **⚠️ Warning**: Deleting the data folder permanently removes all databases, user accounts, and job history. Back up any important data first.
...
**Solution**: Click "More info" then "Run anyway". This is a new, unsigned application — SmartScreen will block it until enough users have run it.
...
|------------|------|---------|
| `AIClusterMaster.exe` | `master/` | Master server |
| `AIClusterWorker.exe` | `worker/` | Worker daemon |
...
| `AIClusterSetup-1.3.1.exe` | — | Installer |
