# AICluster v1.3.1 — Production Deployment Guide
...
| 9876 | Worker ↔ Master heartbeat | Workers | TCP |
...
```text
External :443  →  LB internal :443  (AICluster API)
External :22   →  Jump box :22      (SSH admin — disable password auth)
```
...
5. Failed active master does **not** auto-rejoin — operator intervention required.
...
- Workers are interchangeable — no worker holds exclusive state.
- If a worker disconnects mid-job, the scheduler re-queues the job after `heartbeat_timeout_ms` (default: 30s).
- Idempotent job execution is enforced via job IDs — duplicates are discarded.
...
```powershell
# SQLite pragma tuning — applied at service start
...
|------|----|----------------|----------|
| v1.3.0 | v1.3.1 | Yes | None |
| v1.2.x | v1.3.x | Workers first, then master | ≤ 30s |
...
# 4. Replace binaries
Copy-Item "\\nas01\deploy\aicluster-v1.3.1\worker\*" "C:\Program Files\AICluster\bin\" -Recurse -Force
...
# 5. Replace binaries
Copy-Item "\\nas01\deploy\aicluster-v1.3.1\master\*" "C:\Program Files\AICluster\bin\" -Recurse -Force

# 6. Migrate config (check for new/removed settings)
Copy-Item "\\nas01\deploy\aicluster-v1.3.1\config.yaml" "C:\Program Files\AICluster\config.yaml" -Confirm
...
Copy-Item "\\nas01\deploy\aicluster-v1.3.0\bin\*" "C:\Program Files\AICluster\bin\" -Recurse -Force
Restart-Service AIClusterMaster
Restart-Service AIClusterWorker

# Revert database schema if needed:
& "C:\Program Files\AICluster\tools\sqlite3.exe" "C:\ProgramData\AICluster\cluster.db" ".read 'C:\Program Files\AICluster\migrations\rollback-v1.3.0.sql'"
```
...
Copy-Item "\\nas01\backups\aicluster\binaries\v1.3.1\*" "C:\Program Files\AICluster\" -Recurse -Force

# 4. Restore configuration
Copy-Item "\\nas01\backups\aicluster\config\v1.3.1\config.yaml" "C:\Program Files\AICluster\config.yaml"
...
# 7. Restore model artifacts (selective — only latest production models)
...
# 11. Register workers — for each worker node:
Start-Service AIClusterWorker
```
...
| DB corruption | ≤4 hours (backup interval) | 30 min | Restore DB from latest backup, replay logs |
| Full rack failure | ≤4 hours | 4 hours | Provision new nodes, restore DB + config |
| Region outage | ≤24 hours (off-site backup) | 8 hours | Cross-region deployment, restore from S3 backup |
...
| # | Item | Verified | Notes |
|---|------|----------|-------|
| 1 | Production config uses TLS (port 443, not 8080) | ☐ | Edit `config.yaml` `listen.tls` |
| 2 | Default admin password changed | ☐ | `aicluster-admin.exe user change-password` |
| 3 | API rate limiting enabled | ☐ | `config.yaml` → `rate_limit: 100/1m` |
| 4 | Worker communication uses TLS | ☐ | `config.yaml` → `worker.tls.enabled: true` |
| 5 | Audit logging enabled | ☐ | `config.yaml` → `audit.log_all_requests: true` |
| 6 | Backups verified by restore test | ☐ | Quarterly DR test |
| 7 | Windows Firewall enabled on all profiles | ☐ | `Get-NetFirewallProfile` |
| 8 | No shared accounts used for services | ☐ | Use gMSA or NETWORK SERVICE |
| 9 | SMB signing enabled on storage share | ☐ | `Set-SmbServerConfiguration -RequireSecuritySignature $true` |
| 10 | BitLocker enabled on all data drives | ☐ | `Get-BitLockerVolume` |
| 11 | Log shipping to SIEM configured | ☐ | Winlogbeat / NXLog |
| 12 | Emergency break-glass account exists | ☐ | Local admin with complex password, stored in vault |
...
| Quarterly | Full DR failover test (production — planned window) | SRE |
...
*Document version 1.3.1 — Last updated: 2026-07-04*
